...
const AboutMe = () => {
  return (
    <section id='about' className='container mx-auto px-8 py-16'>
      ...
      <div className="block lg:hidden w-[200px section-title text-left]"></div>
          ...
          <div className="flex gap-4 mt-6">
            {ABOUT_ME.socialLinks.map((item) => (
              <a key={item.label} className="cursor-pointer group" href={item.link} target='_blank' >
                <item.icon className="text-2xl text-[#b56141] transition-transform duration-300 group-hover:rotate-12 group-hover:scale-110 group-hover:translate-y-[-2px] group-hover:text-[#f6a64f]"></item.icon>
              </a>
            ))}
          </div>
